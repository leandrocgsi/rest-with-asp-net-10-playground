...
namespace RestWithASPNET10Erudio.Tests.IntegrationTests.Person.XML
{
    ...
    public class PersonControllerXmlTests : IClassFixture<SqlServerFixture>
    {
        ...
        [Fact(DisplayName = "06 - Find all Person (Paged)")] // ALTERADO
        [TestPriority(6)]
        public async Task FindAllPerson_ShouldReturnPagedResultWithPersons() // ALTERADO
        {
            // Arrange & Act
            var response = await _httpClient
            ...
            var page = await XmlHelper
                .ReadFromXmlAsync<PagedSearchDTO<PersonDTO>>(response); // ALTERADO

            page.Should().NotBeNull(); // ALTERADO
            page.List.Should().NotBeNull(); // NOVO
            page.List.Count.Should().BeGreaterThan(0); // ALTERADO
            ...
            // validações extras da paginação // NOVO
